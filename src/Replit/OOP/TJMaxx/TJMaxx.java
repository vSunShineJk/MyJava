...
public class TJMaxx {
    ...
    public List<String> getAllItemNames() {
        ...
        for(int i = 0; i < regularItems.size()+onSaleItems.size();i++){
            if (i < regularItems.size()) {
                all.add(regularItems.get(i).getName());
            }
            else all.add(onSaleItems.get(i).getName());
        }
    ...
    public double getItemPrice(int catalogNumber) {
        return getItemPrice(catalogNumber);
    }
